...
import { ServiceCreatePlan, ServiceEditPlan } from '@/services/plans';
    ...
    <CardAnimation>
      <Card onClick={() => setOpen(true)}   sx={{maxHeight: 150, maxWidth: 250, borderRadius: 5, border: 1, borderColor: '#DDDDDD'}}>
        ...
        <CardContent sx={{  }} >
          ...
          <Typography sx={{fontWeight: 500, fontSize: '20px', color: '#25282C'}} >{'Create new plan'}</Typography>
        </CardContent>
      </Card>
    </CardAnimation>
...
export function PlanInfo({open, setOpen, edit=false, plan }: {open: boolean, setOpen: any, edit?: boolean, plan?: Plan } ) {
  ...
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
      ...
      try {
          if(!edit){
            const newPlan = await ServiceCreatePlan({Key: planKey, Description: description, Id: 0})
            enqueueSnackbar('Plan created succesful!', { variant: 'success' });
            return setOpen(false)
          }

          const newPlan: Plan & { NewKey: string } = await ServiceEditPlan({Key: plan?.Key as string, Description: description, Id: parseInt(id), NewKey: planKey})
          enqueueSnackbar('Plan updated succesful!', { variant: 'success' });
          return setOpen(false)
